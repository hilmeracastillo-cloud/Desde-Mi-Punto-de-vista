...
export const Chapter6View: React.FC<Chapter6ViewProps> = ({
  fontSizeClass,
  onSelectFootnote,
  onOpenInfographicModal,
  onOpenNotebookInfographicModal,
}) => {
  // Helper to render paragraph with clickable footnote numbers like (68), (69)...
  ...
  const renderInlineText = (text: string) => {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      const footnote = chapter6Footnotes.find((f) => f.id === num);
  ...
  return (
    <div className="space-y-12 max-w-4xl mx-auto animate-fade-in">
      ...
      <div className="bg-[#141414] border border-[#262626] rounded-3xl p-8 sm:p-10 relative overflow-hidden shadow-2xl space-y-4">
        ...
        <div className="flex items-center gap-2">
          <span className="px-3 py-1 rounded-full bg-indigo-950/80 border border-indigo-500/30 text-indigo-400 font-mono text-xs font-bold tracking-wider">
            {chapter6Header.number}
          </span>
          <span className="text-xs font-mono text-slate-400">{chapter6Header.pages}</span>
        </div>

        <h1 className="text-3xl sm:text-5xl font-serif font-bold text-white tracking-tight leading-tight">
          {chapter6Header.title}
        </h1>

        {chapter6Header.subtitle && (
          <h2 className="text-lg sm:text-xl font-serif italic text-indigo-300">
            {chapter6Header.subtitle}
          </h2>
        )}

        {chapter6Header.epigraph && (
        ...
        <div className="pt-2 flex flex-wrap items-center gap-2">
          ...
          <a
            href="#sintesis-conceptos-cap6"
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-purple-950/70 border border-purple-500/40 text-purple-300 hover:text-white hover:bg-purple-600/80 text-xs font-mono font-medium transition-all shadow-sm"
          >
      ...
      {/* Chapter 6 Visual Infographic - NotebookLM Style */}
      {notebookInfographics.cap6 && (
        <NotebookLMInfographic
          data={notebookInfographics.cap6}
          onOpenModal={onOpenNotebookInfographicModal}
        />
      )}

      {/* Chapter 6 Concept Synthesis - Structured Analytical Schema */}
      {chapterInfographics.cap6 && (
        <ChapterInfographic
          data={chapterInfographics.cap6}
          onOpenModal={onOpenInfographicModal}
        />
      )}

      {/* Chapter Footnotes Section */}
      <div id="notas-cap6" className="bg-[#111111] border border-[#222222] rounded-2xl p-6 sm:p-8 space-y-6 shadow-lg scroll-mt-24">
        <div className="flex items-center gap-3 border-b border-[#222222] pb-4">
          ...
          <div>
            ...
            <h2 className="text-2xl font-serif font-bold text-white">Notas del Capítulo 6</h2>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {chapter6Footnotes.map((fn) => (
            <div
              key={fn.id}
      ...
      {/* References Section */}
      <div id="referencias-cap6" className="bg-[#111111] border border-[#222222] rounded-2xl p-6 sm:p-8 space-y-6 shadow-lg scroll-mt-24">
        <div className="flex items-center gap-3 border-b border-[#222222] pb-4">
          <div className="w-8 h-8 rounded-lg bg-indigo-950 border border-indigo-500/30 flex items-center justify-center text-indigo-400">
          ...
          <div>
            ...
            <h2 className="text-2xl font-serif font-bold text-white">Referencias del Capítulo 6</h2>
          </div>
        </div>

        <div className="space-y-3 font-sans text-xs sm:text-sm text-slate-300">
          {chapter6References.map((ref, idx) => (
            <div
              key={idx}
