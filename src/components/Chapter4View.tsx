...
export const Chapter4View: React.FC<Chapter4ViewProps> = ({
  ...
  const handleOpenVisualResource = (type: 'notebook' | 'synthesis') => {
    ...
    setTimeout(() => {
      const targetId = type === 'notebook' ? 'infografia-visual-cap4' : 'sintesis-conceptos-cap4';
  ...
  // Helper to render paragraph with clickable footnote numbers like (44), (45)...
  ...
  const renderInlineText = (text: string) => {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      const footnote = chapter4Footnotes.find((f) => f.id === num);
  ...
  return (
    <div className="space-y-12 max-w-4xl mx-auto animate-fade-in">
      ...
      <div className="bg-[#141414] border border-[#262626] rounded-3xl p-8 sm:p-10 relative overflow-hidden shadow-2xl space-y-4">
        ...
        <div className="flex items-center gap-2">
          <span className="px-3 py-1 rounded-full bg-indigo-950/80 border border-indigo-500/30 text-indigo-400 font-mono text-xs font-bold tracking-wider">
            {chapter4Header.number}
          </span>
          <span className="text-xs font-mono text-slate-400">{chapter4Header.pages}</span>
        </div>

        <h1 className="text-3xl sm:text-5xl font-serif font-bold text-white tracking-tight leading-tight">
          {chapter4Header.title}
        </h1>

        {chapter4Header.subtitle && (
          <h2 className="text-lg sm:text-xl font-serif italic text-indigo-300">
            {chapter4Header.subtitle}
          </h2>
        )}

        {chapter4Header.epigraph && (
        ...
        <div className="pt-2 flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-950/70 border border-emerald-500/40 text-emerald-300 hover:text-white hover:bg-emerald-600/80 text-xs font-mono font-medium transition-all shadow-sm cursor-pointer hover:scale-[1.02]"
          >
      ...
      {/* Chapter 4 Visual Resource Section (renders strictly one at a time, never sequentially) */}
      <ChapterVisualSection
        chapterId="cap4"
        chapterNumber="Capítulo 4"
        notebookData={notebookInfographics.cap4}
        synthesisData={chapterInfographics.cap4}
      ...
      <div id="notas-cap4" className="bg-[#141414] border border-[#262626] rounded-2xl p-6 sm:p-8 space-y-6 shadow-xl scroll-mt-24">
        <div className="flex items-center justify-between border-b border-[#262626] pb-4 flex-wrap gap-3">
          <div className="flex items-center gap-3">
            <Bookmark className="w-5 h-5 text-indigo-400" />
            <div>
              <h2 className="text-xl sm:text-2xl font-serif font-bold text-white">Notas del Capítulo 4 (44 a 60)</h2>
      ...
      {/* References Section */}
      <div id="referencias-cap4" className="bg-[#111111] border border-[#222222] rounded-2xl p-6 sm:p-8 space-y-6 shadow-lg scroll-mt-24">
        <div className="flex items-center gap-3 border-b border-[#222222] pb-4">
          ...
          <div>
            ...
            <h2 className="text-2xl font-serif font-bold text-white">Referencias del Capítulo 4</h2>
          </div>
        </div>

        <div className="space-y-3 font-sans text-xs sm:text-sm text-slate-300">
          {chapter4References.map((ref, idx) => (
            <div
              key={idx}
